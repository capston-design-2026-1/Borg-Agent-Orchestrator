# Windows 전반의 기능 중요도 평가

## 범위

이 보고서는 완성된 기본 고급 XGBoost 모델에 대해 저장된 `feature_importance.json` 아티팩트를 요약합니다.
...
보고된 중요도 값은 각 예측 기간에 대해 모델에서 내보낸 XGBoost 기능 중요도입니다. 이는 모델이 가장 많이 의존하는 입력의 순위를 매기는 데 유용하지만 인과적 설명은 아닙니다.

## 주요 조사 결과

### 1. 모든 창을 지배하는 두 가지 기능
...
이는 기본 고급 모델이 단기 실패 위험을 추정하기 위해 작업 성숙도 및 이벤트 기록 밀도에 크게 의존하고 있음을 의미합니다.
...
| 기능 | 평균 중요도 | 패턴 |
| --- | ---: | --- |
| `task_age_us` | `0.688633` | 모든 지평선에서 지배적이며 지평선 길이가 증가함에 따라 약간 감소합니다 |
...
- 예측 범위가 멀수록 모델은 작업의 더 넓은 맥락에서 누적된 불안정 기록을 더 많이 사용합니다.
...
높은 수준에서 기본 고급 XGBoost 모델은 주로 네 가지 증거 그룹을 사용합니다.
...
- 이러한 중요성은 아직 실행 중인 수리 라벨 조정 재교육이 아닌 저장된 기준 모델에서 비롯됩니다.
