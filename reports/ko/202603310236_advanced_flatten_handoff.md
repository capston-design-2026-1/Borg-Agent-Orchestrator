...
이 보고서는 `~/Documents/borg_xgboost_workspace` 아래에 격리된 고급 XGBoost 파이프라인의 현재 상태를 기록합니다.
...
- 현재 평면화된 고급 샤드 수: `56` 비`.DS_Store` `~/Documents/borg_xgboost_workspace/processed/flat_shards` 아래의 쪽모이 세공 파일

## 최근 행동 변화

- 고급 단계 래퍼는 이제 타임스탬프가 있는 로그를 작성하고 `latest_advanced_*.log` 심볼릭 링크를 유지합니다.
- 이제 플래트너는 0에서 다시 시작하는 대신 기존 샤드 쪽모이 세공 출력에서 ​​다시 시작됩니다.
- 이제 병합기가 다음을 기록합니다.
...
- 현재 이벤트 평면화 경로는 `pl.Object`과 `map_elements(...)`을 통해 중첩된 `resource_request` 값을 추출하는데, 이는 기본 구조체 기반 경로보다 느립니다.
...
3. 평면화가 완료된 후 동일한 격리된 고급 작업 공간을 사용하여 조인, 기능 빌드 및 XGBoost 교육을 계속 진행합니다.