# 모델 디렉터리 레지스트리 및 ML 진화

## 목적

이 보고서는 다음 XGBoost 모델-아티팩트 디렉터리에 대해 설명합니다.
...
- 이는 격리된 고급 XGBoost 파이프라인이 작동하는지 확인하는 데 사용된 초기 스모크 실행이었습니다.
- 최종 모델 품질이 아닌 배관을 검증했습니다.

존재 이유:

- 전체 학습 전에 훨씬 더 작거나 덜 엄격한 실험을 실행하여 다음 사항을 확인하는 것이 표준입니다.
  - 기능 로딩 작업
  - 훈련 작품
  - 유물 수출 작품
  - 메트릭 내보내기가 작동합니다.
...
- 이는 최초의 완전한 다중 수평 고급 XGBoost 모델입니다.
...
- 모든 창에 걸쳐 완전히 조정된 재학습에 시간을 보내기 전에 매개변수 계열을 비교하기 위해 만들어졌습니다.

이 단계가 정상적인 이유:

- 모든 지평선을 즉시 조정하는 것은 비용이 많이 듭니다.
- 일반적인 관행은 하나의 대표적인 지평선을 선택하고, 여러 후보 구성을 실행하고, 성공적인 프로필을 선택하는 것입니다.

결과:

- `regularized_balanced`이 파일럿 스윕에서 승리했으며 나중에 완전히 조정된 재훈련 시도의 기초가 되었습니다.
...
- `tuned_v1`
  - 첫 번째 완전 조정 시도
  - `e/f/g` 수리된 라벨 수정 이전에 시작됨
  - 구식
- `tuned_v2_repaired_labels`
  - 올바른 후속 실행
  - 수리된 `e/f/g` 라벨 포함
  - 현재 조정된 분기를 시청할 수 있습니다.
...
- 격리된 고급 XGBoost 파이프라인이 엔드 투 엔드로 작동했음을 증명
...
- 여러 실패 범위에 대한 심각한 기준 모델 계열을 훈련합니다.
...
- 기능 중요도는 수명주기 및 이벤트 기록 기능에 의해 지배되었습니다.
- 고급 XGBoost 파이프라인은 강력한 내부 순위 지표를 생성할 수 있습니다.
...
- `e/f/g`에 대한 이벤트 플랫 샤드의 조인 키가 손상되었습니다.
...
현재 고급 XGBoost 워크플로는 훈련과 검증 모두에서 네거티브 다운샘플링을 사용합니다. 이는 다음을 의미합니다.
