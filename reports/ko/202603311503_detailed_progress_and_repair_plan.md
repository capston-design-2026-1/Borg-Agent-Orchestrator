...
- 기존 고급 래퍼는 일괄 실행에는 충분했지만 장기 실행 수리 및 재교육 작업에는 너무 부족했습니다.
...
- 현재 문제:
  - 이벤트 플랫 샤드가 존재합니다.
  - 그러나 이벤트 조인 키 열은 기존 쪽모이 세공 파일에서 모두 null입니다.
  - 따라서 결합된 데이터세트에는 이벤트 라벨이 없습니다.
  - 따라서 기능 세트에는 모든 범위에 대해 긍정적인 요소가 없습니다.
...
5. `run_advanced_feature_build_resumable_detailed.sh`을 사용하여 `e/f/g`에 대한 기능 빌드를 다시 실행합니다.
