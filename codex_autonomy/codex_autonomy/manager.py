...
from codex_autonomy.task_store import archive_task, load_tasks, load_tasks_from_dir, move_task, save_task
...
class AutonomyManager:
    ...
    def _defer_cooldown_tasks(self, tasks: list[TaskSpec]) -> None:
        ...
        for task in tasks:
            ...
            moved = move_task(
                task.task_id,
                source_dir=self.config.queue_dir,
                target_dir=self.config.deferred_dir,
            )
            if moved is None:
                continue
            log_event(
                ...
                message=f"moved to deferred queue until {not_before}",
            )

    def _revive_deferred_tasks(self) -> None:
        now_ts = int(time.time())
        for task in load_tasks_from_dir(self.config.deferred_dir):
            not_before = int(task.metadata.get("not_before_epoch", 0) or 0)
            if not_before > now_ts:
                continue
            moved = move_task(
                task.task_id,
                source_dir=self.config.deferred_dir,
                target_dir=self.config.queue_dir,
            )
            if moved is None:
                continue
            log_event(
                ...
                message="cooldown expired; returned to active queue",
            )
    ...
    def run_forever(self) -> None:
        ...
        self.config.deferred_dir.mkdir(parents=True, exist_ok=True)
        ...
        while True:
            ...
            tasks = load_tasks(self.config.queue_dir)
            self._defer_cooldown_tasks(tasks)
