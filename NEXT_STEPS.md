...
- Orchestrator Layer 2 now normalizes AIOpsLab-style nested simulator payloads into the shared `Observation` contract for replay and feature extraction; the AIOpsLab adapter now prefers initialized session state when present, missing node risk/demand scores no longer suppress future-state feature targets with synthetic zeros, and direct `python3` Layer 2 smoke now runs without `numpy` via a narrow compatibility wrapper. Direct upstream package/session validation remains open, and this worktree runtime still cannot run repo `pytest` because `.venv` is a self-referential symlink while fallback `python3` does not have `pytest` installed (2026-04-17 KST).
