...
- 고급 XGBoost 작업 공간 루트: `~/Documents/borg_xgboost_workspace`

두 스크립트 모두 환경 변수로 재정의될 수 있습니다.
...
고급 모델 트랙의 경우 완전히 별도의 작업 공간을 만들어 사용하세요.
...
- 편평하고 파생된 쪽모이 세공 마루는 `~/Documents/borg_xgboost_workspace/processed`으로 이동합니다.
...
`a` 및 `h` 클러스터는 평면화된 사용 스키마가 기본 데이터 세트 그룹과 다르기 때문에 제외됩니다.
...
해당 래퍼는 필요한 경우 고급 작업 공간을 생성하고, 누락된 경우 `~/Documents/borg_xgboost_workspace/config/advanced_xgboost.env`을 생성하고 이를 로드한 다음 일관된 대상 기반 다운로드를 시작합니다.
...
분리된 고급 기능 세트를 구축하고 XGBoost 오류 위험 모델을 교육하려면 다음을 실행하세요.
...
- `~/Documents/borg_xgboost_workspace/processed/feature_store` 아래에 고급 기능 마루를 작성합니다.
- `~/Documents/borg_xgboost_workspace/models/xgboost` 아래에 XGBoost 모델 및 메트릭을 작성합니다.
- 동일한 높은 수준의 목표군을 유지합니다. 구성된 예측 범위 내에서 실패/오류에 대한 위험 점수를 매깁니다.
- 누락된 기능이 있는 레이블 유효 행을 유지하고, 주요 기능에 대해 명시적인 `*_is_missing` 표시기를 추가하며, XGBoost가 조인된 전체 행을 삭제하는 대신 숫자 Null을 누락된 값으로 사용할 수 있도록 합니다.
- 이제 `5`, `15`, `30`, `45` 및 `60` 분에 대한 기본 레이블을 사용하여 동일한 조인된 데이터 세트 및 기능 마루에서 여러 예측 범위를 지원합니다.
...
- `run_advanced_join_resumable.sh`은 조인된 마루가 이미 존재하는 클러스터를 건너뜁니다.
- `run_advanced_feature_build_resumable.sh` 기능 쪽모이 세공이 이미 존재하는 클러스터를 건너뜁니다.
- `run_advanced_train_resumable.sh`은 모델 및 메트릭 아티팩트가 이미 존재하는 목표 범위를 건너뜁니다.
...
로컬 클라우드 원격 측정에서 동일한 표준 스키마를 빌드하려면 쪽모이 세공 마루 또는 CSV 파일과 열 매핑 JSON 파일을 준비하고 다음을 실행합니다.
...
- `temporal_full`은 더 광범위한 실험을 위해 지연, 델타 및 롤링 시간적 기능을 추가합니다.
