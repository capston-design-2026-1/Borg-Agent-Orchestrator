...
| Energy watts | Estimated by the default utilization model. | `est power ...W` is not a physical wattmeter reading. The full calculation is documented in `docs/ENERGY_WATTS_MODEL.md`. |
...
The live exerciser does perform real Kubernetes mutations. For example, it creates bounded BusyBox CPU-burner Deployments such as `light-dvfs` and `safety-replicate`, plus unschedulable admission-pressure Deployments such as `admission-queue`, under `borg-orchestrator-exercise`. Those Deployments create real Metrics Server CPU movement, scheduler outcomes such as `Pending` pods, and `FailedScheduling` events. The dashboard therefore mixes:
...
| Agent A/B/C proposals, referee choice, reward totals | Orchestrator control-plane decisions evaluated through the twin/reward path and, in live exercise mode, executed against the experimental exercise namespace |
| Energy watts | Model-derived estimate using `idle_watts=80`, `cpu_full_scale_watts=120`, `mem_full_scale_watts=60` unless calibrated otherwise. See `docs/ENERGY_WATTS_MODEL.md`. |
...
| Control pressure timelines | Four separate rolling five-minute graph widgets: Safety Forecast, Admission Backlog, Efficiency Energy, and Reward Outcome. The comparison dashboard polls about once per second, while the server allows retained samples at `0.5` second spacing, so short-lived control changes are easier to inspect. `Efficiency Energy` plots controlled-namespace dynamic watts for experimental and baseline using the same local utilization-derived model, so the controller-relevant CPU/memory work can be compared without unrelated control-plane and Prometheus noise. The separate Agent B reward-power chip remains the experimental `agent_energy_watts` signal. |
