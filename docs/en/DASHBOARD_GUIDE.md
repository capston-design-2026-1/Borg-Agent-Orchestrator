...
| Control pressure timelines | Four separate rolling five-minute graph widgets: Safety Forecast, Admission Backlog, Efficiency Energy, and Reward Outcome. `Efficiency Energy` now plots experimental `comparison_power_watts` and baseline estimated watts together using the same local utilization-derived model, so the two clusters can be compared directly. The separate Agent B reward-power chip remains the experimental `agent_energy_watts` signal. |
